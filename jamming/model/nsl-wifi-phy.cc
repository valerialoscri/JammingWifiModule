...
NslWifiPhy::DriverStartTx (Ptr<const Packet> packet, double txPower)
{
  ...
  NS_LOG_DEBUG ("Ns" << m_state->GetDelayUntilIdle ());
                                ...
                                uint16_t preamble)
{
  ...
  WifiPreamble  preamble2;
   ...
   NS_LOG_FUNCTION(this << packet << rxPowerW << rxDuration);
    ...
    {
        if(preamble == 10){
            NS_LOG_FUNCTION("test preamble");
            preamble2= WifiPreamble::WIFI_PREAMBLE_INVALID;
         }
    else{
     ...
     preamble2 = tag.GetPreambleType (); 
   WifiModulationClass modulation = tag.GetModulation ();
   WifiTxVector txVector;
   txVector.SetPreambleType (preamble2);
   if ((modulation == WIFI_MOD_CLASS_DSSS) || (modulation == WIFI_MOD_CLASS_HR_DSSS))
   ...
   else if (modulation == WIFI_MOD_CLASS_VHT)
     {
       ...
       vhtSigHdr.SetMuFlag (preamble2 == WIFI_PREAMBLE_VHT_MU);
    ...
    if (rxPowerW > -140)
      {
        ...
        if (preamble2 == WIFI_PREAMBLE_INVALID)
          {
...
NslWifiPhy::EndReceive (Ptr<Packet> packet, Ptr<Event> event)
{
  ...
  NS_LOG_DEBUG("testjskd");
  WifiTxVector txVector = event->GetTxVector();
  if(txVector.IsAggregation()){
    NS_LOG_DEBUG("aggregarer");
    ...
    Time remainingAmpDuration = event ->GetEndTime()- event->GetStartTime();
    ...
    for(const auto & subframe: ampduSubframes){
      ...
      --nbOfRemainingMpdus;
      if(nbOfRemainingMpdus == 0 && !remainingAmpDuration.IsZero())
      {
        mpduDuration += remainingAmpDuration;
      }
  ...
  {
    NS_LOG_INFO("asds");
    NotifyMonitorSniffRx(packet, (uint16_t)GetChannelFrequencyMhz (),txVector,signalNoise,statusPerMpdu);
 ...
 /* if(rxInfo.first){
      ...
      NS_LOG_DEBUG(event->GetPreambleType ());
       ...
       // double noiseDbm = RatioToDb (event->GetRxPowerW () / snrPer.snr) - GetRxNoiseFigure () + 30;
        ...
        NS_LOG_DEBUG("testsss");
        NS_LOG_(statusPerMpdu);
        NS_LOG_DEBUG(event->GetTxVector());
        m_state->SwitchFromRxEndOk (packet, snrPer.snr, event->GetTxVector(),statusPerMpdu);
       NS_LOG_DEBUG("test1");
      DriverEndRx (copy, snrPer.packetRss, true);
      NS_LOG_DEBUG("test");
