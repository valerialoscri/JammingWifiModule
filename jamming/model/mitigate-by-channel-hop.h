...
public:
  ...
  /**
   * List of detection mitigation method.
   */
  enum JammingMitigationMethod {
    FREQUENCY =0, // beta mitigation +1
    RANDOM,       // randomly choose new channel
    RL,            // reinforcementLearning
    SCAN          // scan all channel and switch on the best
  };
...
public:
  ...
  void SetJammingMitigationMethod(JammingMitigationMethod method);
  uint32_t GetJammingMitigationMethod (void) const;
  ...
  void BeginMitigationMethod(void) const;
  /**
  ...
  void DoMitigationConstant(void);
...
private:
  ...
  virtual void ChangeChildChannel(uint32_t channel);
...
//  virtual void DoEndTxHandler (Ptr<Packet> packet, double txPower);
  ...
  uint16_t ElementaryGenerator (void);

  void BeginRLMitigation(void);

  void GetObservationsRl(void);

  uint16_t Scan(void);

  void Test();

  uint32_t GetNumberDetection(void);
...
private:
  ...
  /**
   * Jamming mitigation method to use.
   */

  JammingMitigationMethod m_jammingMitigationMethod; 
  ...
  /***Boolean RL***/
  bool m_mitigation_RL;

  /**NUmber observation **/

  uint32_t m_observation;

  /**Number current observation **/

  uint32_t m_currentObservation;

  uint32_t m_numberDetection;
};
