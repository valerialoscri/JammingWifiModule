  ...
  static TypeId tid = TypeId ("ns3::ReactiveJammer")
    ...
    .AddAttribute ("ReactiveJammerReactToMitigation",
     ...
     .AddAttribute ("ReactByChannelHopMethod",
                     ...
                     UintegerValue (2), // default to FREQUENCY
                     MakeUintegerAccessor (&ReactiveJammer::SetJammingReactMethod,
                                           &ReactiveJammer::GetJammingReactMethod),
                      MakeUintegerChecker<uint32_t> ())
...
void 
ReactiveJammer::SetJammingReactMethod(JammingReactMethod method){
    m_jammingReactMethod = method; 
}

uint32_t 
ReactiveJammer::GetJammingReactMethod (void) const {
  ...
  if (m_reactToMitigation)
    {
      ...
      NS_LOG_DEBUG ("ReactiveJammer:At Node #" <<  m_rxTimeout );         
...
ReactiveJammer::RxTimeoutHandler (void)
{
  ...
  uint16_t currentChannel = m_utility->GetPhyLayerInfo ().currentChannel;
   uint16_t nextChannel = m_utility->GetPhyLayerInfo ().currentChannel;
   switch (m_jammingReactMethod)
    {
    case FREQUENCY:
    ...
    case RL:
      ...
      break;
    default:
       nextChannel = m_utility->GetPhyLayerInfo ().currentChannel;
      break;
    }
...
uint16_t
ReactiveJammer::RandomChannel (void){
...
void
ReactiveJammer::BeginRLMitigation(){
   NS_LOG_FUNCTION (this);
   uint32_t openGymPort = 5555;
   Ptr<OpenGymInterface> openGymInterface = CreateObject<OpenGymInterface> (openGymPort);
   m_myGymEnv= CreateObject<MyGymEnv>(m_utility->GetPhyLayerInfo ().currentChannel,12);
   m_myGymEnv->SetOpenGymInterface(openGymInterface);
  }

uint16_t
ReactiveJammer::GetObservationsRl(){
     std::ostringstream oss;
     oss.str ("");
     oss << "/NodeList/";
     double pdr =   m_utility->GetPdr ();
     m_myGymEnv->PerformCca(m_myGymEnv,m_utility->GetPhyLayerInfo ().currentChannel,pdr);
...
void 
ReactiveJammer::Optimal(uint32_t optimal){
